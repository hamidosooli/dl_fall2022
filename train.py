...
def plot_mean_with_std(ls, chunk_length, **kwargs):
    ...
    max_reward = np.zeros_like(means)
    # plt.plot(means - scale*stds, lw=0.5, c='#396AB1')
    # plt.plot(means + scale*stds, lw=0.5, c='#396AB1')
    # plt.plot(max_reward, lw=1, color='#CC2529')
    plt.fill_between(range(len(means)),means - scale*stds, means + scale*stds, color=c, alpha=0.2)

    global p1
    # plt.fill(means, color=c, alpha=0.5)
    p1=plt.plot(means, lw=1, color=c)
...
def plot_rewards(agent_rewards, average_window):
    ...
    for i, reward_list in enumerate(agent_rewards):
        plot_mean_with_std(agent_rewards[i][:], int(average_window), scale=2.5, color=colors[i])
    ...
    plt.legend([p1[0], p1[0], p1[0], p1[0]], ['Max Reward', 'Agent 1', 'Agent 2', 'Agent 3'])

    leg = ax.get_legend()
    leg.legendHandles[0].set_color('#CC2529')
    leg.legendHandles[1].set_color('#396AB1')
    leg.legendHandles[2].set_color('#DA7C30')
    leg.legendHandles[3].set_color('#3E9651')
